...
const { PDFDocument, rgb } = require('pdf-lib'); // We'll install pdf-lib just in case
...
const docs = [
    ...
    { title: "Tema 3 - Reglamento del Congreso.pdf", url: "https://www.boe.es/buscar/pdf/1982/BOE-A-1982-5365-consolidado.pdf" }, // Adjusted ID
    ...
    { title: "Tema 8 - Ley 6-1997 LOFAGE.pdf", url: "https://www.boe.es/buscar/pdf/1997/BOE-A-1997-7878-consolidado.pdf" }, // Might not have consolidated
    ...
    { title: "Tema 12 - RGPD.pdf", url: "https://www.boe.es/buscar/pdf/2016/DOUE-L-2016-80807.pdf" }, // Just guessing it might be on BOE
    ...
    for (const doc of docs) {
        ...
        try {
            ...
            if (error.response && error.response.status === 404) {
                console.log(`❌ 404 Not Found for ${doc.title}. Trying original URL...`);
                try {
                    // Try without -consolidado
                    const origUrl = doc.url.replace('-consolidado', '');
                    const resOrig = await axios({
                        method: 'GET',
                        url: origUrl,
                        responseType: 'arraybuffer',
                        timeout: 10000,
                    ...
                    if (resOrig.status === 200 && resOrig.headers['content-type'].includes('pdf')) {
                        fs.writeFileSync(destPath, resOrig.data);
                        console.log(`✅ Success (Original): ${doc.title}`);
                    } else {
                        throw new Error("Not a PDF");
                    }
                } catch (e2) {
                    console.log(`❌ Failed both for ${doc.title}. Creating mock...`);
                    await createMockPDF(doc.title, destPath);
                }
            } else {
                console.log(`❌ Error ${error.message} for ${doc.title}. Creating mock...`);
                await createMockPDF(doc.title, destPath);
            }
        }
    }
